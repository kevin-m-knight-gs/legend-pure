...
public class TestElementLoader extends AbstractReferenceTest
{
    ...
    public static void serialize() throws IOException
    {
        ...
        referenceIds = ReferenceIds.builder(processorSupport).withAvailableExtensions().build();
        fileSerializer = FileSerializer.builder()
                .withFilePathProvider(FilePathProvider.builder().withLoadedExtensions().build())
                .withSerializers(ConcreteElementSerializer.builder(processorSupport).withLoadedExtensions().withReferenceIds(referenceIds).build(), ModuleMetadataSerializer.builder().withLoadedExtensions().build())
                .build();
        ...
        metadataIndex = MetadataIndex.builder()
                .withModules(allModules.asLazy().collect(m -> fileSerializer.deserializeModuleMetadata(serializationDir, m)))
                .build();
    }
    ...
    private void testVirtualPackage(String path)
    {
        ...
        executeElementLoaderTest(loader ->
        {
            ...
            Assert.assertTrue(path, loaded instanceof FakeVirtualPackage);
            FakeVirtualPackage virtualPkg = (FakeVirtualPackage) loaded;
    ...
    private void testConcreteElement(String path, String classifierPath)
    {
        ...
        executeElementLoaderTest(loader ->
        {
            ...
            DeserializedConcreteElement expectedDeserialized = fileSerializer.deserializeElement(serializationDir, path);
    ...
    public void testTopLevelsAndPackaged()
    {
        executeElementLoaderTest(loader -> elementsByPath.forEachKeyValue((path, e) ->
        {
            ...
            else
            {
                ...
                Assert.assertTrue(path, loaded instanceof FakeVirtualPackage);
                FakeVirtualPackage virtualPackage = (FakeVirtualPackage) loaded;
    ...
    public void testConcurrency()
    {
        ...
        executeElementLoaderTest(loader ->
        {
            ...
            Assert.assertEquals(elementsByPath, result.getOne().collect((path, e) -> Tuples.pair(path, ((FakeElement<?>) e).getInstance())));
    ...
    {
        test.accept(ElementLoader.fromDirectory(metadataIndex, elementBuilder, referenceIds, fileSerializer, serializationDir));
        ...
        {
            test.accept(ElementLoader.fromClassLoader(metadataIndex, elementBuilder, referenceIds, fileSerializer, classLoader));
        }
        catch (IOException e)
        {
    ...
    private static class FakeElementBuilder implements ElementBuilder
    {
        ...
        @Override
        public CoreInstance buildVirtualPackage(VirtualPackageMetadata metadata, MetadataIndex index, ReferenceIds referenceIds)
        {
            this.virtualPackageCounter.incrementAndGet();
            return new FakeVirtualPackage(metadata);
        }

        @Override
        public CoreInstance buildConcreteElement(ConcreteElementMetadata metadata, MetadataIndex index, ReferenceIds referenceIds, Supplier<? extends DeserializedConcreteElement> deserializer)
        {
    ...
    private static class FakeElement<M extends PackageableElementMetadata> extends AbstractCoreInstanceWrapper
    {
        private final M metadata;

        protected FakeElement(M metadata)
        {
    ...
    private static class FakeVirtualPackage extends FakeElement<VirtualPackageMetadata>
    {
        private FakeVirtualPackage(VirtualPackageMetadata metadata)
        {
            super(metadata);
        }
    }

    private static class FakeConcreteElement extends FakeElement<ConcreteElementMetadata>
    {
