...
public class ModuleMetadataSerializerV1 implements ModuleMetadataSerializerExtension
{
    ...
    private void writeElement(Writer writer, ConcreteElementMetadata element)
    {
        ...
        {
            String refId = pair.getOne();
            ImmutableList<GraphPath> refPaths = pair.getTwo();
            writer.writeString(refId);
            writer.writeInt(refPaths.size());
            refPaths.forEach(refPath -> writeGraphPathEdges(writer, refPath));
        });
    }
    ...
    private ConcreteElementMetadata readElement(Reader reader)
    {
        ...
        for (int i = 0; i < extRefCount; i++)
        {
            ...
            GraphPath[] refPaths = new GraphPath[pathCount];
            for (int j = 0; j < pathCount; j++)
            {
                refPaths[j] = readGraphPathEdges(reader, path);
            }
            builder.withExternalReferences(refId, refPaths);
        }
    ...
    private void writeGraphPathEdges(Writer writer, GraphPath path)
    {
    ...
    private GraphPath readGraphPathEdges(Reader reader, String startNodePath)
    {
        int edgeCount = reader.readInt();
        GraphPath.Builder builder = GraphPath.builder(edgeCount, false)
                .withStartNodePath(startNodePath);
        for (int i = 0; i < edgeCount; i++)
        {
            readGraphPathEdge(reader, builder);
        }
        return builder.build();
    }

    private void readGraphPathEdge(Reader reader, GraphPath.Builder builder)
    {
