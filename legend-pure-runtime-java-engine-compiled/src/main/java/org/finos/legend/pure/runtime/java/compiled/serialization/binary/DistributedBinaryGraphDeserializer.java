...
public abstract class DistributedBinaryGraphDeserializer
{
    ...
    private static class Many extends DistributedBinaryGraphDeserializer
    {
        ...
        protected Obj getInstance(String classifierId, String instanceId, boolean throwIfNotFound)
        {
            MutableList<Obj> objs = this.deserializers.collect(d -> d.getInstanceIfPresent(classifierId, instanceId), Lists.mutable.ofInitialCapacity(this.deserializers.size()));
            objs.removeIf(Objects::isNull);
            if (objs.isEmpty())
            {
                if (throwIfNotFound)
                {
            ...
            return mergeAndPossiblyValidate(objs);
        }
        ...
        protected ListIterable<Obj> getInstances(String classifierId, Iterable<String> instanceIds, boolean throwIfNotFound)
        {
            ...
            return objsById.valuesView().collect(this::mergeAndPossiblyValidate, Lists.mutable.ofInitialCapacity(objsById.size()));
        }

        private Obj mergeAndPossiblyValidate(ListIterable<Obj> objs)
        {
            Obj obj = (objs.size() == 1) ? objs.get(0) : Obj.merge(objs);
            if (this.validateObjs)
            {
                validateFullyDefined(obj);
            }
