...
public class TestLineInfo extends AbstractPureTestWithCoreCompiledPlatform
{
    ...
    @Test
    public void testLineInfoForStaticInstanceGeneratedM3()
    {
        ...
        new M3AntlrParser().parse("Class Person\n" +
                             ...
                             "   lastName:String[1];\n" +
                             "}\n" +
                             "^Person p ?[a/b/c/file.txt:1,3,2,2,4,5]? (lastName='last')\n", "fromString.pure", false, 0, repository, parsed, VoidM3M4StateListener.VOID_M3_M4_STATE_LISTENER, context, 1, null);
        repository.validate(new VoidM4StateListener());
        PostProcessor.process(parsed, repository, new ParserLibrary(), new InlineDSLLibrary(), null, context, processorSupport, null, null);
        Validator.validateM3(parsed, ValidationType.DEEP, new ParserLibrary(), new InlineDSLLibrary(), runtime.getCodeStorage(), repository, context, processorSupport);

        CoreInstance p = runtime.getCoreInstance("p");
        Assert.assertNotNull(p);
        assertSourceInformation("a/b/c/file.txt", 1, 3, 2, 2, 4, 5, p.getSourceInformation());

        CoreInstance person = runtime.getCoreInstance("Person");
    ...
    @Test
    public void testLineInfoForDynamicInstanceGeneratedM3()
    {
        compileTestSource("fromString.pure",
                          ...
                          "}\n");
        // Test class
        CoreInstance person = runtime.getCoreInstance("pkg1::Person");
        Assert.assertNotNull(person);
        assertSourceInformation("fromString.pure", 1, 1, 1, 13, 9, 1, person.getSourceInformation());

        MapIterable<String, CoreInstance> propertiesByName = processorSupport.class_getSimplePropertiesByName(person);
        MapIterable<String, CoreInstance> qualifiedPropertiesByName = _Class.getQualifiedPropertiesByName(person, processorSupport);
        ...
        assertSourceInformation("/platform/pure/grammar/m3.pure", 1136, 13, 1136, 126, 1143, 17, classifierGenericType.getSourceInformation());
        ...
        ListIterable<? extends CoreInstance> funcExpressions = Instance.getValueForMetaPropertyToManyResolved(func, M3Properties.expressionSequence, processorSupport);
        ...
        CoreInstance newKlp = Instance.getValueForMetaPropertyToManyResolved(expression, M3Properties.parametersValues, processorSupport).get(1);
        ...
        // Note: these source coordinates may change if m3.pure changes
        assertSourceInformation("/platform/pure/grammar/m3.pure", 2376, 1, 2376, 88, 2400, 1, funcType.getSourceInformation());
    }

    @Test
    public void testLineInfoForImportGroup()
    {
        compileTestSource("testImportGroup.pure", "import apps::something::*;\n" +
                                             "//comment\n" +
                                             ...
                                             "   testClass;\n" +
                                             "}\n");
        CoreInstance testInstance1 = runtime.getCoreInstance("my::test::testClass");
        CoreInstance testInstance2 = runtime.getCoreInstance("my::test::getTestClass__Any_MANY_");
