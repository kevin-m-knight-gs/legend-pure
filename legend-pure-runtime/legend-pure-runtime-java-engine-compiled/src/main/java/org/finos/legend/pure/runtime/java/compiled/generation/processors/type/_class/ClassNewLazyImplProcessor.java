...
public class ClassNewLazyImplProcessor
{
    ...
    private static StringJavaSource buildConcreteElementImplementation(String javaPackage, CoreInstance classGenericType, CoreInstance _class, ProcessorContext processorContext, ProcessorSupport processorSupport)
    {
        ...
        Class<AbstractLazyConcreteElement> superClass = AbstractLazyConcreteElement.class;
        ...
        if (simpleProperties.anySatisfy(propertyInfo -> propertyInfo.canBePrimitive() && !M3Properties.name.equals(propertyInfo.name)))
        {
            appendGetToCoreInstanceFunction(builder.append('\n'));
        }
    ...
    private static StringJavaSource buildComponentInstanceImplementation(String javaPackage, CoreInstance classGenericType, CoreInstance _class, ProcessorContext processorContext, ProcessorSupport processorSupport)
    {
        ...
        Class<AbstractLazyCoreInstance> superClass = AbstractLazyCoreInstance.class;
        ...
        appendValidate(builder, classGenericType, _class, interfaceNamePlusTypeParams, simpleProperties, processorContext);
        if (simpleProperties.anySatisfy(PropertyInfo::canBePrimitive))
        {
            appendGetToCoreInstanceFunction(builder.append('\n'));
        }
        return StringJavaSource.newStringJavaSource(javaPackage, className, builder.append("}\n").toString());
    ...
    private static StringJavaSource buildVirtualPackageImplementation(String javaPackage, CoreInstance classGenericType, CoreInstance _class, ProcessorContext processorContext, ProcessorSupport processorSupport)
    {
        ...
        Class<AbstractLazyPackageableElement> superClass = AbstractLazyPackageableElement.class;
    ...
    private static StringBuilder appendStandardMethods(StringBuilder builder, ListIterable<PropertyInfo> simpleProperties, Class<? extends AbstractLazyCoreInstance> superClass)
    {
        ...
        appendAddKeyValue(builder, simpleProperties, isConcreteElement, isPackageableElement).append('\n');
        return appendPrint(builder);
    }
    ...
    private static StringBuilder appendConcreteElementConstructor(StringBuilder builder, String className)
    {
        return builder.append("    public ").append(className).append("(ConcreteElementMetadata metadata, MetadataIndex index, ElementBuilder elementBuilder, ReferenceIdResolvers referenceIds, PrimitiveValueResolver primitiveValueResolver, Supplier<? extends DeserializedConcreteElement> deserializer)\n")
                .append("    {\n")
                .append("        super(null, -1, metadata, index, elementBuilder, referenceIds, primitiveValueResolver, deserializer);\n")
                .append("    }\n");
    }
    ...
    {
        builder.append("    public ").append(className).append("(InstanceData instanceData, MetadataIndex index, ReferenceIdResolver referenceIdResolver, IntFunction<CoreInstance> internalIdResolver, PrimitiveValueResolver primitiveValueResolver, ElementBuilder elementBuilder)\n")
                .append("    {\n")
                .append("        super(null, -1, (instanceData.getName() == null) ? \"Anonymous_NoCounter\" : instanceData.getName(), instanceData.getSourceInformation(), instanceData.getCompileStateBitSet(), instanceData.getClassifierPath(), referenceIdResolver);\n");
        if (simpleProperties.notEmpty())
        {
            ...
            {
                builder.append("        collectBackReferences(instanceData.getReferenceId(), index, referenceIdResolver, internalIdResolver, elementBuilder");
    ...
    {
        builder.append("    public ").append(className).append("(VirtualPackageMetadata metadata, MetadataIndex index, ReferenceIdResolvers referenceIds, ElementBuilder elementBuilder)\n");
        builder.append("    {\n");
        builder.append("        super(null, -1, null, CompileStateSet.PROCESSED_VALIDATED.toBitSet(), metadata, referenceIds);\n");
        MutableSet<String> backRefProperties = Sets.mutable.empty();
        simpleProperties.forEach(propertyInfo ->
        {
            if (propertyInfo.backRef)
        ...
        if (backRefProperties.notEmpty())
        {
            builder.append("        collectBackReferences(metadata.getPath(), index, referenceIds, elementBuilder");
            M3PropertyPaths.BACK_REFERENCE_PROPERTY_PATHS.collect(ImmutableList::getLast, Lists.mutable.ofInitialCapacity(M3PropertyPaths.BACK_REFERENCE_PROPERTY_PATHS.size()))
                    .sortThis()
                    .forEach(p -> builder.append(", ").append(backRefProperties.contains(p) ? p : "null"));
            builder.append(");\n");
    ...
    private static StringBuilder appendPrint(StringBuilder builder)
    {
        return builder.append("    @Override\n")
                .append("    public void print(Appendable appendable, String tab, int max)\n")
                .append("    {\n")
                .append("        ConsoleCompiled.print(appendable, tab, this, max);\n")
                .append("    }\n");
    }

    private static void appendGetToCoreInstanceFunction(StringBuilder builder)
    {
        builder.append("    private java.util.function.Function<Object, ? extends CoreInstance> getToCoreInstanceFunction()\n")
                .append("    {\n")
                .append("        return new java.util.function.Function<Object, CoreInstance>()\n")
                .append("        {\n")
                .append("            @Override\n")
                .append("            public CoreInstance apply(Object o)\n")
                .append("            {\n")
                .append("                return ValCoreInstance.toCoreInstance(o);\n")
                .append("            }\n")
                .append("        };\n")
                .append("    }\n");
    }
    ...
    {
        builder.append("    @Override\n")
                .append("    protected void initialize(InstanceData instanceData, MetadataIndex index, ReferenceIdResolver referenceIdResolver, IntFunction<CoreInstance> internalIdResolver, PrimitiveValueResolver primitiveValueResolver, ElementBuilder elementBuilder)\n")
                .append("    {\n");
        MutableSet<String> backRefProperties = Sets.mutable.empty();
        simpleProperties.forEach(propertyInfo ->
        {
