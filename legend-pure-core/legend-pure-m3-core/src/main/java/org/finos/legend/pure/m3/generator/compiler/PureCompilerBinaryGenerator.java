...
public class PureCompilerBinaryGenerator
{
    ...
    public static void serializeModules(Path outputDirectory, ClassLoader classLoader, Iterable<String> modules, Iterable<String> excludedModules)
    {
        ...
        try
        {
            CodeRepositorySet repositorySet = resolveRepositories(moduleSet, excludedModules);
            PureRuntime runtime = compile(currentClassLoader, repositorySet);
            serialize(outputDirectory, moduleSet, runtime);
        }
        catch (Throwable t)
        {
    ...
    private static CodeRepositorySet resolveRepositories(SetIterable<String> modules, Iterable<String> excludedModules)
    {
        RichIterable<CodeRepository> foundRepos = CodeRepositoryProviderHelper.findCodeRepositories(true);
    ...
    private static void serialize(Path outputDirectory, SetIterable<String> modules, PureRuntime runtime)
    {
        long start = System.nanoTime();
