...
public class TestPureCompilerSerializer extends AbstractReferenceTest
{
    ...
    public void testSerializeAllToDirectory() throws IOException
    {
        ...
        {
            Assert.assertTrue(moduleName, fileDeserializer.moduleMetadataExists(directory, moduleName));
            Assert.assertEquals(moduleName, moduleInfo.metadata, fileDeserializer.deserializeModuleMetadata(directory, moduleName));
    ...
    public void testSerializeAllToJar() throws IOException
    {
        ...
        try (URLClassLoader classLoader = new URLClassLoader(new URL[]{jarPath.toUri().toURL()}, null))
        {
            getAllModuleInfos().forEachKeyValue((moduleName, moduleInfo) ->
            {
                Assert.assertTrue(moduleName, fileDeserializer.moduleMetadataExists(classLoader, moduleName));
                Assert.assertEquals(moduleName, moduleInfo.metadata, fileDeserializer.deserializeModuleMetadata(classLoader, moduleName));
                moduleInfo.elements.forEachKeyValue((path, element) ->
                {
        ...
        baseModuleInfos.forEachKeyValue((moduleName, moduleInfo) ->
        {
            ...
            if (expectedSources.notEmpty())
            {
                Assert.assertEquals(moduleName,
                        ...
                        fileDeserializer.deserializeModuleMetadata(directory, moduleName));
            }
            else
            {
                Assert.assertFalse(moduleName, fileDeserializer.moduleMetadataExists(directory, moduleName));
            }
        });
    }
    ...
    {
        Assert.assertFalse(path, fileDeserializer.elementExists(directory, path));
    }
    ...
    {
        Assert.assertFalse(path, fileDeserializer.elementExists(classLoader, path));
    }
