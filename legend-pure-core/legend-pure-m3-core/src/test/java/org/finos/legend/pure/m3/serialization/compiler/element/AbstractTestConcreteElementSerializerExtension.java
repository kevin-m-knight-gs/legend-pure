...
public abstract class AbstractTestConcreteElementSerializerExtension extends AbstractReferenceTest
{
    ...
    private void assertDeserialization(DeserializedConcreteElement concreteElement, MutableIntObjectMap<CoreInstance> internalReferences, String path, CoreInstance element, InstanceData instanceData)
    {
            ...
            if (values.notEmpty() &&
                    (!M3PropertyPaths.BACK_REFERENCE_PROPERTY_PATHS.contains(realKey) ||
                            M3Properties.referenceUsages.equals(key) &&
                                    values.anySatisfy(ru -> !refUsageHasExternalOwner(ru, concreteElement.getConcreteElementData().getSourceInformation()))))
            {
        ...
        nonEmptyPropertyKeys.forEach(key ->
        {
            ...
            {
                values = values.reject(ru -> refUsageHasExternalOwner(ru, concreteElement.getConcreteElementData().getSourceInformation()));
            }
