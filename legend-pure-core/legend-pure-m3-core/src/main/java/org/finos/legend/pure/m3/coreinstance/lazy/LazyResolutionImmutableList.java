...
abstract class LazyResolutionImmutableList<T> extends AbstractImmutableCollection<T> implements ImmutableList<T>, List<T>, RandomAccess
{
    ...
    private LazyResolutionImmutableList(AtomicReferenceArray<Object> items)
    {
        this.items = items;
    }
    ...
    public void forEachWithIndex(int fromIndex, int toIndex, ObjectIntProcedure<? super T> procedure)
    {
        ...
        int arrayFrom = toArrayIndex(fromIndex);
        int arrayTo = toArrayIndex(toIndex);
        if (fromIndex <= toIndex)
        {
            for (int i = arrayFrom; i <= arrayTo; i++)
            {
                procedure.value(getResolvedByArrayIndex(i), i);
            }
        }
        else
        {
    ...
    public ImmutableList<T> toReversed()
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            Object item = this.items.get(arrayIndex);
            if (item.getClass() == SharedSupplier.class)
            {
                ...
                if (supplier.isResolved())
                {
                    ...
                    this.items.compareAndSet(arrayIndex, item, value);
                    reversed[targetIndex] = value;
                }
                else
                {
                    anySupplier = true;
    ...
    public ImmutableList<T> newWithout(T element)
    {
        ...
        {
            newItems[i] = this.items.get(toArrayIndex(i));
        }
        boolean anySupplier = false;
        ...
        for (int i = toArrayIndex(0); i < end; i++)
        {
            ...
            if (item.getClass() == SharedSupplier.class)
            {
                ...
                if (supplier.isResolved())
                {
                    ...
                    this.items.compareAndSet(i, item, value);
    ...
    {
        private SimpleLazyResolutionImmutableList(AtomicReferenceArray<Object> items)
        {
            super(items);
        }
    ...
    private static class LazyResolutionImmutableSubList<T> extends LazyResolutionImmutableList<T>
    {
        ...
        private LazyResolutionImmutableSubList(AtomicReferenceArray<Object> items, int fromIndex, int toIndex)
        {
            super(items);
    ...
    {
        AtomicReferenceArray<Object> items = new AtomicReferenceArray<>(suppliers.size());
        suppliers.forEachWithIndex((supplier, i) -> items.set(i, new SharedSupplier<>(supplier)));
