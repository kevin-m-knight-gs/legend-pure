...
public class ModuleMetadata
{
    ...
    public String toString()
    {
        ...
        this.elements.forEach(e -> builder.append(e.getPath()).append(", "));
    ...
    public static class Builder
    {
        ...
        private static <T, V extends Comparable<? super V>> void sortAndRemoveDuplicates(MutableList<T> list, Function<T, V> keyFn, String description)
        {
            if (list.size() > 1)
            {
                list.sortThisBy(keyFn::apply);
                T[] prev = (T[]) new Object[1];
                V[] prevKey = (V[]) new Comparable[1];
                list.removeIf(current ->
                {
                    T previous = prev[0];
                    V key = keyFn.apply(current);
                    if ((previous == null) || !Objects.equals(key, prevKey[0]))
                    {
                        prev[0] = current;
                        prevKey[0] = key;
                        return false;
