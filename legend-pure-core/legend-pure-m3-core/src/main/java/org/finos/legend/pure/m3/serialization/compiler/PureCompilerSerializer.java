...
public class PureCompilerSerializer
{
    ...
    public void serializeElement(Path directory, String elementPath)
    {
        CoreInstance element = this.processorSupport.package_getByUserPath(Objects.requireNonNull(elementPath, "Invalid element path: null"));
        if (element == null)
        {
            throw new IllegalArgumentException("Unknown element: " + elementPath);
        }
        this.fileSerializer.serializeElement(directory, element);
        String moduleName = ModuleHelper.getElementModule(element);
        if (moduleName != null)
        {
            updateModuleMetadata(directory, moduleName, Lists.immutable.with(elementPath), Sets.immutable.with(element.getSourceInformation().getSourceId()));
        }
    }


    public void serializeElements(Path directory, String... elementPaths)
    {
        switch (elementPaths.length)
        {
            case 0:
            {
                return;
            ...
            case 1:
            {
                serializeElement(directory, elementPaths[0]);
                return;
            }
            default:
            {
                serializeElements(directory, Sets.mutable.with(elementPaths));
            }
        }
    }

    public void serializeElements(Path directory, Iterable<? extends String> elementPaths)
    {
        SetIterable<? extends String> elementPathSet = toElementSet(elementPaths);
        if (elementPathSet.contains(null))
        {
            throw new NullPointerException("Invalid element path: null");
        }
        switch (elementPathSet.size())
        {
            case 0:
            {
                return;
            ...
            case 1:
            {
                serializeElement(directory, elementPathSet.getAny());
                return;
            }
            default:
            {
                MutableMap<String, Pair<MutableList<String>, MutableSet<String>>> elementsAndSourcesByModule = Maps.mutable.empty();
                elementPathSet.forEach(elementPath ->
                {
                    CoreInstance element = this.processorSupport.package_getByUserPath(elementPath);
                    if (element == null)
                    {
                        throw new IllegalArgumentException("Unknown element: " + elementPath);
                    }
                    ...
                    if (moduleName != null)
                    {
                        Pair<MutableList<String>, MutableSet<String>> pair = elementsAndSourcesByModule.getIfAbsentPut(moduleName, () -> Tuples.pair(Lists.mutable.empty(), Sets.mutable.empty()));
                        pair.getOne().add(elementPath);
                        pair.getTwo().add(element.getSourceInformation().getSourceId());
                        this.fileSerializer.serializeElement(directory, element);
                    }
                });
                elementsAndSourcesByModule.forEachKeyValue((moduleName, moduleElementsAndSources) ->
                        updateModuleMetadata(directory, moduleName, moduleElementsAndSources.getOne(), moduleElementsAndSources.getTwo()));
            }
        }
    }

    public void serializeElements(Path directory, Predicate<? super CoreInstance> elementFilter)
    {
        ...
        {
            String moduleName = ModuleHelper.getElementModule(element);
            if ((moduleName != null) && elementFilter.test(element))
            {
                Pair<MutableList<String>, MutableSet<String>> pair = elementsAndSourcesByModule.getIfAbsentPut(moduleName, () -> Tuples.pair(Lists.mutable.empty(), Sets.mutable.empty()));
                pair.getOne().add(PackageableElement.getUserPathForPackageableElement(element));
                pair.getTwo().add(element.getSourceInformation().getSourceId());
                this.fileSerializer.serializeElement(directory, element);
            }
        });
    ...
    private void serializeModule(Serializer serializer, String moduleName)
    {
        ...
        try
        {
            GraphTools.getTopLevelAndPackagedElements(this.processorSupport)
                    .select(e -> ModuleHelper.isElementInModule(e, moduleName))
                    .forEach(serializer::serializeElement);
            serializer.serializeModuleMetadata(this.moduleMetadataGenerator.generateModuleMetadata(ModuleHelper.resolveModuleName(moduleName)));
        }
        catch (Throwable t)
        {
    ...
    private void serializeModules(Serializer serializer, SetIterable<? extends String> moduleNames)
    {
        ...
        switch (moduleNames.size())
        {
            ...
            default:
            {
                ...
                {
                    GraphTools.getTopLevelAndPackagedElements(this.processorSupport)
                            .select(e -> moduleNames.contains(ModuleHelper.getElementModule(e)))
                            .forEach(serializer::serializeElement);
    ...
    private void updateModuleMetadata(Path directory, String moduleName, ListIterable<String> elementsToUpdate, SetIterable<String> sourcesToUpdate)
    {
        ModuleMetadata initial = this.fileSerializer.moduleMetadataExists(directory, moduleName) ?
                                 this.fileSerializer.deserializeModuleMetadata(directory, moduleName) :
                                 ModuleMetadata.builder(moduleName).build();
        ModuleMetadata updated = this.moduleMetadataGenerator.update(initial, elementsToUpdate, sourcesToUpdate);
        this.fileSerializer.serializeModuleMetadata(directory, updated);
    }

    private static SetIterable<? extends String> toElementSet(Iterable<? extends String> elementPaths)
    {
        if (elementPaths instanceof SetIterable)
        {
            return (SetIterable<? extends String>) elementPaths;
        }
        if (elementPaths instanceof Set)
        {
            return SetAdapter.adapt((Set<? extends String>) elementPaths);
        }
        return Sets.mutable.withAll(elementPaths);
    }
    ...
    private static class DirectorySerializer implements Serializer
    {
        ...
        {
            this.fileSerializer.serializeModuleMetadata(this.directory, moduleMetadata);
        }
    }
    ...
    private static class ZipStreamSerializer implements Serializer
    {
        ...
        public void serializeModuleMetadata(ModuleMetadata moduleMetadata)
        {
            this.fileSerializer.serializeModuleMetadata(this.stream, moduleMetadata);
        }
    }
}
