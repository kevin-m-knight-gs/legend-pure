...
public class TestModuleMetadataSerializer
{
    ...
    public void testSerializeByVersion()
    {
        ...
        serializer.serialize(BinaryWriters.newBinaryWriter(stream), emptyMetadata, 1);
        ...
        serializer.deserialize(BinaryReaders.newBinaryReader(stream.toByteArray()));
        Assert.assertEquals(1, v1Serialize.getCount());
        ...
        stream.reset();
        serializer.serialize(BinaryWriters.newBinaryWriter(stream), emptyMetadata, 2);
        Assert.assertNotEquals(0, stream.size());
        Assert.assertEquals(1, v1Serialize.getCount());
        Assert.assertEquals(1, v1Deserialize.getCount());
        ...
        stream.reset();
        serializer.serialize(BinaryWriters.newBinaryWriter(stream), emptyMetadata);
        Assert.assertNotEquals(0, stream.size());
        Assert.assertEquals(1, v1Serialize.getCount());
        Assert.assertEquals(1, v1Deserialize.getCount());
        ...
        stream.reset();
        IllegalArgumentException e = Assert.assertThrows(IllegalArgumentException.class, () -> serializer.serialize(BinaryWriters.newBinaryWriter(stream), emptyMetadata, 3));
    ...
    private ModuleMetadataSerializerExtension newExtension(int version, BiConsumer<? super Writer, ? super ModuleMetadata> serializer, Function<? super Reader, ? extends ModuleMetadata> deserializer)
    {
        return new ModuleMetadataSerializerExtension()
        {
            ...
            public void serialize(Writer writer, ModuleMetadata metadata)
            {
                ...
                serializer.accept(writer, metadata);
            }
