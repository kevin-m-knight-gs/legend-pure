...
class SerializerV1 extends BaseV1
{
    ...
    private Writer collectNodesAndIndexStrings(Writer writer)
    {
        ...
        {
            if (propertyInfo.category == PropertyCategory.REF_USAGE)
            {
                ListIterable<? extends CoreInstance> internalReferenceUsages = node.instance.getValueForMetaPropertyToMany(propertyInfo.name)
                        // visited is the set of internal nodes we collected in the previous loop
                ...
                if (internalReferenceUsages.notEmpty())
                {
                    ...
                    strings.add(propertyInfo.name);
                    strings.add(propertyInfo.sourceType);
                    strings.add(this.serializationContext.getReferenceIdProvider().getReferenceId(refUsageClass));
    ...
    private void serializeNode(Writer writer, NodeToSerialize node, ObjectIntMap<CoreInstance> internalIds, int internalIdWidth)
    {
        ...
        getPropertyInfos(node.classifier).forEach(propertyInfo ->
        {
            switch (propertyInfo.category)
            {
                ...
                case REF_USAGE:
                {
                    ListIterable<? extends CoreInstance> internalReferenceUsages = node.instance.getValueForMetaPropertyToMany(propertyInfo.name).select(internalIds::containsKey);
                    if (internalReferenceUsages.notEmpty())
                    {
                        propertiesWithValues.add(Tuples.pair(propertyInfo, internalReferenceUsages));
                    }
                    break;
                }
            }
    ...
    {
        PropertyCategory propertyCategory = getPropertyCategory(name, property);
        String sourceType = (propertyCategory == PropertyCategory.OTHER_BACK_REF) ? null : getClassifierPath(Property.getSourceType(property, this.processorSupport));
        return new PropertyInfo(name, sourceType, propertyCategory);
    }

    private PropertyCategory getPropertyCategory(String propertyName, CoreInstance property)
    {
        switch (propertyName)
        {
            case M3Properties.referenceUsages:
            {
                // Note that we handle referenceUsages differently than other back reference properties
                return M3PropertyPaths.referenceUsages.equals(Property.calculatePropertyPath(property, this.processorSupport)) ? PropertyCategory.REF_USAGE : PropertyCategory.ORDINARY;
            }
            case M3Properties.applications:
            {
                return M3PropertyPaths.applications.equals(Property.calculatePropertyPath(property, this.processorSupport)) ? PropertyCategory.OTHER_BACK_REF : PropertyCategory.ORDINARY;
            }
            case M3Properties.modelElements:
            {
                return M3PropertyPaths.modelElements.equals(Property.calculatePropertyPath(property, this.processorSupport)) ? PropertyCategory.OTHER_BACK_REF : PropertyCategory.ORDINARY;
            }
            case M3Properties.propertiesFromAssociations:
            {
                return M3PropertyPaths.propertiesFromAssociations.equals(Property.calculatePropertyPath(property, this.processorSupport)) ? PropertyCategory.OTHER_BACK_REF : PropertyCategory.ORDINARY;
            }
            case M3Properties.qualifiedPropertiesFromAssociations:
            {
                return M3PropertyPaths.qualifiedPropertiesFromAssociations.equals(Property.calculatePropertyPath(property, this.processorSupport)) ? PropertyCategory.OTHER_BACK_REF : PropertyCategory.ORDINARY;
            }
            case M3Properties.specializations:
            {
                return M3PropertyPaths.specializations.equals(Property.calculatePropertyPath(property, this.processorSupport)) ? PropertyCategory.OTHER_BACK_REF : PropertyCategory.ORDINARY;
            }
            default:
            {
    ...
    {
        SourceInformation sourceInfo = instance.getSourceInformation();
        return (sourceInfo != null) && !this.element.getSourceInformation().subsumes(sourceInfo);
    }
    ...
    private static class PropertyInfo
    {
        ...
        private PropertyInfo(String name, String sourceType, PropertyCategory category)
        {
            this.name = name;
    ...
    {
        ORDINARY, REF_USAGE, OTHER_BACK_REF
    }
}
