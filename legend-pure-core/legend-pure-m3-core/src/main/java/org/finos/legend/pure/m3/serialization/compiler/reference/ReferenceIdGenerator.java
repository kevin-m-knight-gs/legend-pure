...
{
    private static final ImmutableSet<ImmutableList<String>> SKIP_PROPERTY_PATHS = M3PropertyPaths.BACK_REFERENCE_PROPERTY_PATHS.newWith(M3PropertyPaths._package);
    ...
    public MutableMap<CoreInstance, String> generateIdsForElement(String path)
    {
    ...
    public MutableMap<CoreInstance, String> generateIdsForElement(CoreInstance element)
    {
        validateIsPackageableElement(Objects.requireNonNull(element, "element may not be null"));
        return generateIdsForElement(element, org.finos.legend.pure.m3.navigation.PackageableElement.PackageableElement.getUserPathForPackageableElement(element));
    }

    private MutableMap<CoreInstance, String> generateIdsForElement(CoreInstance element, String path)
    {
        validateElementAndPath(element, path);
        return new Generator(element, path, this.processorSupport).generateIds();
    }

    private void validateIsPackageableElement(CoreInstance element)
    {
        if (!(element instanceof PackageableElement) && ((element instanceof Any) || !this.processorSupport.instance_instanceOf(element, M3Paths.PackageableElement)))
        {
            ...
            else
            {
                ...
                org.finos.legend.pure.m3.navigation.PackageableElement.PackageableElement.writeUserPathForPackageableElement(builder, classifier);
            }
            throw new IllegalArgumentException(builder.toString());
        }
    }

    private void validateElementAndPath(CoreInstance element, String path)
    {
    ...
    private static class Generator
    {
        ...
        private Generator(CoreInstance element, String path, TypeCache typeCache)
        {
            ...
            enqueue(GraphPath.buildPath(path), Lists.immutable.with(element), this.typeCache.getClassifier(element));
        }

        private Generator(CoreInstance element, String path, ProcessorSupport processorSupport)
        {
            this(element, path, new TypeCache(processorSupport));
        }

        private MutableMap<CoreInstance, String> generateIds()
        {
        ...
        private void advanceFromSearchNode(SearchNode searchNode)
        {
            ...
            {
                // Ignore properties whose type is primitive
                if (this.typeCache.isPrimitiveType(propertyInfo.rawType))
                {
                    return;
                }

                // Skip back reference properties, package, etc
                ListIterable<String> realKey = instance.getRealKeyByName(propertyName);
                if (SKIP_PROPERTY_PATHS.contains(realKey))
                {
                    return;
                }
                ...
                if (propertyInfo.isToOne)
                {
                    ...
                    if ((value != null) && isInternal(value) && !searchNode.pathNodes.contains(value))
                    {
                        ...
                        {
                            GraphPath newGraphPath = searchNode.path.withToOneProperty(propertyName);
                ...
                if (values.notEmpty())
                {
                    ...
                    PropertyIndex index = tryIndex(propertyInfo, realKey, values);
                    if (index == null)
                    {
                        values.forEachWithIndex((value, i) ->
                        {
                            if (isInternal(value) && !pathNodes.contains(value))
                            {
                                ...
                                if (!this.typeCache.isPrimitiveType(valueClassifier))
                                {
                                    GraphPath newGraphPath = searchNode.path.withToManyPropertyValueAtIndex(propertyName, i);
                                    ImmutableList<CoreInstance> newPathNodes = searchNode.pathNodes.newWith(value);
                                    enqueue(newGraphPath, newPathNodes, valueClassifier);
                                }
                            }
                    ...
                    else
                    {
                        index.index.forEachKeyValue((name, value) ->
                        {
                            if (isInternal(value) && !pathNodes.contains(value))
                            {
                                ...
                                CoreInstance valueClassifier = this.typeCache.getClassifier(value);
                                GraphPath newGraphPath = searchNode.path.withToManyPropertyValueWithKey(propertyName, index.property, name);
                                ImmutableList<CoreInstance> newPathNodes = searchNode.pathNodes.newWith(value);
                                enqueue(newGraphPath, newPathNodes, valueClassifier);
                            }
                        });
        ...
        private PropertyIndex tryIndex(PropertyInfo propertyInfo, ListIterable<String> realKey, ListIterable<? extends CoreInstance> values)
        {
            // Check that the property type is concrete and non-primitive
            if ((propertyInfo.rawType == null) || this.typeCache.isPrimitiveType(propertyInfo.rawType))
            {
                return null;
            }

            // Figure out which key to index by and check that it's present, to-one, and has type of String
            String keyProp = getIndexKey(realKey);
            PropertyInfo keyPropInfo = this.typeCache.getSimplePropertyInfo(propertyInfo.rawType).get(keyProp);
            if ((keyPropInfo == null) || !keyPropInfo.isToOne || !this.typeCache.isStringType(keyPropInfo.rawType))
            {
                return null;
            }
        ...
        private static String getIndexKey(ListIterable<String> realKey)
        {
            if (M3PropertyPaths.qualifiedProperties.equals(realKey))
            {
                return M3Properties.id;
            }
            if (M3PropertyPaths.p_stereotypes.equals(realKey) || M3PropertyPaths.p_tags.equals(realKey))
            {
    ...
    private static class TypeCache
    {
        ...
        private final MutableMap<CoreInstance, MapIterable<String, PropertyInfo>> propertyInfoByClassByName = Maps.mutable.empty();
        ...
        private MapIterable<String, PropertyInfo> computeSimplePropertyInfo(CoreInstance classifier)
        {
            ...
            properties.forEachKeyValue((name, prop) -> map.put(name, computePropertyInfo(classGenericType, prop)));
            if (classifier == this.enumerationClass)
            {
                ...
                if ((propInfo != null) && (propInfo.rawType == null))
                {
                    map.put(M3Properties.values, new PropertyInfo(this.processorSupport.package_getByUserPath(M3Paths.Enum), propInfo.isToOne));
                }
            }
            return map;
        }

        private PropertyInfo computePropertyInfo(CoreInstance classGenericType, CoreInstance property)
        {
            ...
            boolean isToOne = Multiplicity.isToOne(resolvedMultiplicity, false);

            return new PropertyInfo(rawType, isToOne);
        }
    }
    ...
    private static class PropertyInfo
    {
        ...
        private PropertyInfo(CoreInstance rawType, boolean isToOne)
        {
