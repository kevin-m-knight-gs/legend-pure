...
public class TestTDSCompiled extends TestTDS
{
    ...
    public Object getValueAsCoreInstance(String columnName, int rowNum)
    {
        ...
        switch (this.columnType.get(columnName))
        {
            case INT:
            {
                ...
                return !isNull[rowNum] ? (long) value : null;
            }
            case CHAR:
            {
                char[] data = (char[]) dataAsObject;
                return !isNull[rowNum] ? Character.toString(data[rowNum]) : null;
            }
            case STRING:
            {
            ...
            case DOUBLE:
            {
                ...
                return !isNull[rowNum] ? data[rowNum] : null;
            }
            default:
            {
    ...
    public TestTDSCompiled updateColumns(ProcessorSupport processorSupport)
    {
        ...
        MutableList<CoreInstance> columns = this.columnsOrdered.collect(c -> (CoreInstance) _Column.getColumnInstance(c,false, typeParam, (GenericType) processorSupport.type_wrapGenericType(_Package.getByUserPath(convert(columnType.get(c)), processorSupport)), null, processorSupport)).toList();
