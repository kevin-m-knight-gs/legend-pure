...
class ExternalReferenceIndex
{
    ...
    static ExternalReferenceIndex buildIndex(ElementIndex elementIndex)
    {
        ...
        elementIndex.forEachElement(e -> e.getExternalReferences().forEachKeyValue((refId, refPaths) -> refPaths.collect(p -> new ExternalReference(p, refId), initialIndex.getIfAbsentPut(refId, () -> Lists.mutable.ofInitialCapacity(refPaths.size())))));
    ...
    private static int compareGraphPaths(GraphPath path1, GraphPath path2)
    {
        ...
        cmp = Integer.compare(len, path2.getEdgeCount());
