...
public abstract class AbstractLazyConcreteElement extends AbstractLazyPackageableElement
{
    ...
    protected AbstractLazyConcreteElement(ModelRepository repository, int internalSyntheticId, ConcreteElementMetadata metadata, MetadataIndex index, ElementBuilder elementBuilder, ReferenceIdResolvers referenceIds, PrimitiveValueResolver primitiveValueResolver, Supplier<? extends DeserializedConcreteElement> deserializer)
    {
        super(repository, internalSyntheticId, metadata.getSourceInformation(), 0, metadata, referenceIds);
        this.init = new Init(index, elementBuilder, referenceIds, primitiveValueResolver, deserializer);
    }
            ...
            synchronized (local)
            {
                if (this.init != null)
                {
                    ...
                    try
                    {
                        ...
                        IntFunction<CoreInstance> intIdResolver = newInternalIdResolver(deserialized, local.elementBuilder, local.index, refIdResolver);
                        ...
                        initialize(concreteElementData, local.index, refIdResolver, intIdResolver, local.primitiveValueResolver, local.elementBuilder);
                        this.init = null;
                    }
                    catch (Throwable t)
                    {
    ...
    protected abstract void initialize(InstanceData instanceData, MetadataIndex index, ReferenceIdResolver referenceIdResolver, IntFunction<CoreInstance> internalIdResolver, PrimitiveValueResolver primitiveValueResolver, ElementBuilder elementBuilder);
    ...
    private static class Init
    {
        ...
        private Init(MetadataIndex index, ElementBuilder elementBuilder, ReferenceIdResolvers referenceIds, PrimitiveValueResolver primitiveValueResolver, Supplier<? extends DeserializedConcreteElement> deserializer)
        {
            this.index = index;
