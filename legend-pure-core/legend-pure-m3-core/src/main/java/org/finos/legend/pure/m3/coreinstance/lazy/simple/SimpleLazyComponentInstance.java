...
class SimpleLazyComponentInstance extends AbstractLazyCoreInstance implements CoreInstanceWithStandardPrinting
{
    ...
    SimpleLazyComponentInstance(ModelRepository repository, int internalSyntheticId, InstanceData instanceData, ListIterable<? extends BackReference> backReferences, ReferenceIdResolver referenceIdResolver, IntFunction<? extends CoreInstance> internalIdResolver, PrimitiveValueResolver primitiveValueResolver, ElementBuilder elementBuilder)
    {
        super(repository, internalSyntheticId, resolveName(instanceData, repository), instanceData.getSourceInformation(), instanceData.getCompileStateBitSet(), instanceData.getClassifierPath(), referenceIdResolver);
